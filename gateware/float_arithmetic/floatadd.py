...
#        self.source = source = Source(EndpointDescription(rgb16f_layout(dw), packetized=True))
        ...
        for name in ["r", "g", "b"]:
            ...
            PipelinedActor.__init__(self, self.datapath.latency)
            self.comb += self.datapath.ce.eq(self.pipe_ce)
            self.comb += getattr(self.datapath.sink, "in1").eq(getattr(sink, name + "1"))
            self.comb += getattr(self.datapath.sink, "in2").eq(getattr(sink, name + "2"))
#            self.comb += getattr(self.datapath.sink, "in2").eq(0) # 0.0
