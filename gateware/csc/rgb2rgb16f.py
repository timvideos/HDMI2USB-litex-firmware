...
class RGB2RGB16fDatapath(Module):
    def __init__(self, rgb_w, rgb_16f_w):
        self.sink = sink = Record(rgb_layout(rgb_w))
        self.source = source = Record(rgb_16f_layout(rgb16f_w))
        ...
        self.sync += [
            ...
            r_frac_val[10:3].eq(sink.r[7:0])
            g_frac_val[10:3].eq(sink.g[7:0])
            b_frac_val[10:3].eq(sink.b[7:0])
        ...
        self.sync += [
            ...
            source.r[:11].eq(r_frac_val << r_lshift_val)
            source.r[11:16].eq(15 - 1 - r_lshift_val)
            source.r[16].eq(0)

            source.g[:11].eq(g_frac_val << g_lshift_val)
            source.g[11:16].eq(15 - 1 - g_lshift_val)
            source.g[16].eq(0)

            source.b[:11].eq(b_frac_val << b_lshift_val)
            source.b[11:16].eq(15 - 1 - b_lshift_val)
            source.b[16].eq(0)
